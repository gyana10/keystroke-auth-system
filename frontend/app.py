...
if page == "Project Info":
    ...
    st.header("🏢 Data Warehouse Metrics")
    ...
    try:
        ...
        with col1:
            st.metric(
                "Total Records",
                int(metrics["total_records"][0])
            )

        with col2:
            st.metric(
                "Avg Feature 1",
                round(metrics["avg_feature_1"][0], 2)
            )

        with col3:
            st.metric(
                "Avg Feature 2",
                round(metrics["avg_feature_2"][0], 2)
            )
...
# =====================================================
    ...
    stats_df = get_user_statistics()

    metrics_df = get_typing_metrics()
    ...
    fig2 = px.bar(
        ...
        x="Feature",
        y="Average Value",
        title="Average Keystroke Features"
    )
